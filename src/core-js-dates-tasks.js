...
function getNextFriday(date) {
  ...
  nextFriday.setDate(
    nextFriday.getDate() + (daysTillFriday === 0 ? 7 : daysTillFriday)
  );
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber = Math.ceil(((tempDate - start) / 86400000 + 1) / 7);
...
function getNextFridayThe13th(date) {
  ...
  const unluckyWeekday = 5;
  const unluckyDate = new Date(date);
  unluckyDate.setDate(13);
  while (unluckyDate.getDay() !== unluckyWeekday) {
    unluckyDate.setMonth(month);
    month += 1;
  }
  return unluckyDate;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (workDay <= end) {
    let i = 0;
    while (i < countWorkDays && workDay <= end) {
      schedule.push(new Date(workDay));
